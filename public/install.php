    ...
    if ($currentStep === 2) {
        ...
        if (!$errors) {
            try {
                ...
                $pdo->exec('CREATE TABLE IF NOT EXISTS meldescheine (
                    ...
                    signature_token VARCHAR(191) NULL,
                    signature_token_expires_at DATETIME NULL,
                    signature_token_created_at DATETIME NULL,
                ...
                $pdo->exec('CREATE TABLE IF NOT EXISTS reservation_items (
                    ...
                    CONSTRAINT fk_install_reservation_items_primary_guest FOREIGN KEY (primary_guest_id) REFERENCES guests(id) ON DELETE SET NULL,
        ...
        if (
            ...
            if ($reservationId > 0) {
                $sampleNights = 1;
                try {
                    $interval = (new DateTimeImmutable($arrival))->diff(new DateTimeImmutable($departure));
                    $sampleNights = max(1, (int) $interval->days);
                } catch (Exception $exception) {
                    $sampleNights = 1;
                }
            ...
            if ($overbookingReservationId > 0) {
                $itemInsert = $pdo->prepare('INSERT INTO reservation_items (reservation_id, category_id, room_id, rate_id, room_quantity, occupancy, primary_guest_id, arrival_date, departure_date, price_per_night, total_price, created_at, updated_at) VALUES (?, ?, NULL, NULL, ?, ?, ?, ?, ?, NULL, NULL, NOW(), NOW())');
                $itemInsert->execute([
